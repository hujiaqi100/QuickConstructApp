...
export class DataProcess {
  private static isEmpty = (value: null | undefined): boolean => {
    return value === null || value === undefined;
  }

  public static getReflect = (result: any[], hf: H_Form, formName: string) => {
    const { config, setConfig }: any = hf.getConfig(formName);
  ...
  public static isWrappedWithDollarSigns = (str: string): boolean => {
    const regex = /^\$.*\$$/;
    return regex.test(str);
  }

  public static removeSigns = (element: any): any => {
  ...
  public static reset = (data: any[], tree: any[], name = '') => {
    if (!Array.isArray(data) && typeof data === 'object') {
      Object.keys(data).forEach(val => {
        ...
        const node = DataProcess.traceTree(newName.slice(1), tree);
        if (node) {
        ...
        DataProcess.reset(data[val as any], tree, newName);
      });
    }
